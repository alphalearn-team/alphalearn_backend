...
public class ProfilePictureStorageService {
    ...
    private final ProfilePictureObjectKeyFactory objectKeyFactory;
    ...
    public ProfilePictureStorageService(
            ...
            ProfilePictureObjectKeyFactory objectKeyFactory,
    ...
    public PresignedUpload generatePresignedUpload(UUID learnerId, String originalFilename, String contentType) {
        requireEnabled();

        String objectKey = objectKeyFactory.buildObjectKey(learnerId, originalFilename);
    ...
    public String expectedObjectKeyPrefix(UUID learnerId) {
        return "profile-pictures/%s/".formatted(learnerId);
    }
    ...
    private void requireEnabled() {
        if (!properties.enabled() || presignerProvider.getIfAvailable() == null || s3ClientProvider.getIfAvailable() == null) {
            throw new IllegalStateException("Profile picture storage is not enabled");
        }
    }
