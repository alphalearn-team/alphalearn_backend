...
class LearnerGameMonthlyPackServiceTest {
    ...
    private LearnerGameMonthlyPackService service;

    @BeforeEach
    void setUp() {
        Clock clock = Clock.fixed(Instant.parse("2026-04-15T00:00:00Z"), ZoneOffset.UTC);
        service = new LearnerGameMonthlyPackService(
    ...
    void returnsExistsFalseWhenCurrentMonthPackMissing() {
        ...
        LearnerCurrentGameMonthlyPackDto result = service.getCurrentMonthlyPack(learnerAuthUser());
    ...
    private SetupData setupPackData() {
        ...
        List<GameMonthlyPackConcept> conceptRows = java.util.stream.IntStream.range(0, concepts.size())
                .mapToObj(index -> packConcept(pack, concepts.get(index), (short) (index + 1)))
                .toList();

        List<GameMonthlyPackWeeklyFeature> weeklyFeatures = List.of(
    ...
    private GameMonthlyPackConcept packConcept(GameMonthlyPack pack, Concept concept, short slotIndex) {
    ...
    private record SetupData(List<GameMonthlyPackWeeklyFeature> weeklyFeatures) {
    }
}
